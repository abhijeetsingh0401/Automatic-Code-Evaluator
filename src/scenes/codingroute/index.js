...
const CodingRoute = () => {
  ...
  useEffect(() => {
    if (subject, codingQuestion) {
      fetchDocumentData(subject, codingQuestion);
    }
  }, [subject, codingQuestion]);

  const fetchDocumentData = async (subject, codingQuestion) => {
    setLoading(true);
    try {
      const docRef = doc(db, subject,'CodingQuestion');
      const docSnap = await getDoc(docRef);

      if (docSnap.exists()) {
        const tempdata = docSnap.data();
        const data = tempdata[codingQuestion]
        console.log(data)
